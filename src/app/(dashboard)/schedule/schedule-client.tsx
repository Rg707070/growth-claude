...
// ─── Types ────────────────────────────────────────────────────────────────────
type CalendarView = 'today' | 'daily' | 'weekly' | 'monthly' | 'yearly' | 'all' | 'habit-calendar' | 'google-calendar'
...
// ─── YearlyView ───────────────────────────────────────────────────────────────
// ─── GoogleCalendarView ─────────────────────────────────────────────────────
function buildGoogleEmbedSrc(raw: string): string {
  ...
  return (
    <div className="flex flex-col" style={{ height: 'calc(100dvh - 64px)' }}>
      ...
      <div className="flex-shrink-0 max-w-2xl mx-auto w-full px-4 pb-2">
        <div className="flex gap-1 p-1 rounded-2xl" style={{ background: w(0.05, isDark) }}>
          {([
            ...
            { id: 'google-calendar' as const, label: isRTL ? 'גוגל'      : 'Google'   },
          ]).map(tab => (
            <button
              key={tab.id}
      ...
      {view === 'google-calendar' && (
        <div className="flex-1 min-h-0 overflow-y-auto max-w-2xl mx-auto w-full pb-24"
          style={{ borderTop: `1px solid ${w(0.06, isDark)}` }}>
          <GoogleCalendarView isDark={isDark} isRTL={isRTL} />
        </div>
      )}
